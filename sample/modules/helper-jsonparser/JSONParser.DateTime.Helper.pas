unit JSONParser.DateTime.Helper;

interface

uses System.sysUtils, System.DateUtils;

type TJSONParserDatetimeHelper = record helper for TDateTime
...
{ TJSONParserDatetimeHelper }

function TJSONParserDatetimeHelper.DateTimeToIso8601: string;
begin
  if Self = 0 then
    Result := ''
  else
...
function TJSONParserDatetimeHelper.Format(ADateFormat: String): String;
begin
  result := FormatDateTime(ADateFormat, Self);
end;

function TJSONParserDatetimeHelper.FormatYYYY_MM_DD: string;
begin
  result := Format('yyyy-MM-dd');
end;

procedure TJSONParserDatetimeHelper.fromIso8601ToDateTime(AValue: String);
begin
  Self := Iso8601ToDateTime(AValue);
end;

function TJSONParserDatetimeHelper.Iso8601ToDateTime(AValue: String): TDateTime;
var
