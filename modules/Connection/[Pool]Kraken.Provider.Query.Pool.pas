unit Kraken.Provider.Query.Pool;
...
type
  ...
  public
    { public declarations }
    procedure DoGetInstance(var AInstance: T; var AInstanceOwner: Boolean; const AId: Integer = -1); virtual; abstract;
    ...
    ///<summary>
    ///  Usado para pegar conexão especificando um identificador, somente caso não exista, será instanciado novo
    ///</summary>
    function TryGetItemById(const Value: Integer): TPoolItem<T>;
  end;
...
function TPoolManager<T>.TryGetItemById(const Value: Integer): TPoolItem<T>;
var
  I: Integer;
  LPoolItem: TPoolItem<T>;
  LInstance: T;
...
begin
  ...
  try
    for I := 0 to Pred(FPoolItemList.Count) do
    begin
      if FPoolItemList.Items[I].Id = Value then
      begin
        Result := FPoolItemList.Items[I];
        Break;
      end;
    ...
    begin
      try
        LInstance := nil;
        LInstanceOwner := False;
        DoGetInstance(LInstance, LInstanceOwner, Value);
      finally
        if LInstance <> nil then
        begin
          LPoolItem := TPoolItem<T>.Create(LInstance, LInstanceOwner);
